        ...
        if (state.action === 'awaiting_link_qrcode' && text) {
            ...
            try {
                ...
                await bot.sendMessage(chatId, `✅ **Pengaturan Selesai!**\n\n` +
                    `Simulasi Anda telah dibuat dengan ID: \`${simulationId}\`\n\n` +
                ...
                console.error("Gagal membuat simulasi doxware:", error);
                bot.sendMessage(chatId, "Terjadi kesalahan saat membuat simulasi. Silakan coba lagi.");
            } finally {
            ...
            if (!simulation) {
                bot.sendMessage(chatId, "Simulasi tidak lagi valid atau telah kedaluwarsa.");
                delete userStates[chatId];
                return;
            }
    ...
    if (data === "product") showCategories(chatId);
      ...
      handleBuyProduct(chatId, userId, productId);
        ...
        const premiumButtons = [
            { text: "🎓 Doxware Simulation", callback_data: "doxware_simulation_start" },
...
- \`C:\\Users\\User\\Photos\\liburan_2024.jpg\` -> \`liburan_2024.jpg.locked\`
